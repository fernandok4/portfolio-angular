...
@Component({
  selector: 'app-about-skill-one-page',
  templateUrl: './about-skill.component.html',
  styleUrls: ['./about-skill.component.scss']
})
export class AboutSkillOnePageComponent implements OnInit {
  ...
  private createCamera = () => {
    let arcRotateCamera = new ArcRotateCamera("camera", -Math.PI / 2, Math.PI / 2.5, 7, new Vector3(0, 0, 0), this.scene)
    arcRotateCamera.lowerRadiusLimit = arcRotateCamera.upperRadiusLimit = arcRotateCamera.radius;
    (arcRotateCamera.inputs.attached.pointers as any).buttons.splice(1, 2)
  ...
  private getSpherePoints = (arrayHeight, sphere) => {
    ...
    for(let i = 0; i < heightDesired.length; i++){
      ...
      let vectorPositive = new Vector3(spherePivot.x + polarRadius * widthFactorX, spherePivot.y + height, spherePivot.z)
      let vectorNegative = new Vector3(spherePivot.x + polarRadius * widthFactorX, spherePivot.y - height, spherePivot.z)
